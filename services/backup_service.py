...
def _openssl_encrypt(data: bytes, passphrase: str) -> bytes:
    ...
    try:
        ...
        raise BackupError("openssl is missing in this environment; cannot create an encrypted backup") from exc
...
def _openssl_decrypt(data: bytes, passphrase: str) -> bytes:
    ...
    try:
        ...
        raise BackupError("openssl is missing in this environment; cannot decrypt the backup") from exc
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or b"").decode("utf-8", errors="replace").strip()
...
class CloudflareR2Client:
    ...
    def validate(self) -> None:
        ...
        if missing:
            raise BackupError(f"R2 config incomplete: missing {', '.join(missing)}")
...
class BackupService:
    ...
    def delete_backup(self, key: str) -> None:
        ...
        if not candidate:
            raise BackupError("Backup object key must not be empty")
        client = CloudflareR2Client(config.get_backup_settings())
        try:
    ...
    def download_backup(self, key: str) -> dict[str, object]:
        ...
        if candidate.endswith(".enc"):
            ...
            if not passphrase:
                raise BackupError("No encryption passphrase configured; cannot download and decrypt an encrypted backup")
    ...
    def _decode_backup_payload(self, key: str, payload: bytes) -> dict[str, object]:
        ...
        if key.endswith(".enc"):
            passphrase = _clean(config.get_backup_settings().get("passphrase"))
            if not passphrase:
                raise BackupError("No encryption passphrase configured; cannot view an encrypted backup")
    ...
    def _decode_archive_detail(self, payload: bytes) -> dict[str, object]:
        ...
        try:
            ...
            raise BackupError("Failed to parse the backup archive; it may be corrupted") from exc

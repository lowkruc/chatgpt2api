...
class ChatRequirements:
    """Holds the sentinel tokens needed for one conversation request."""
    token: str
    proof_token: str = ""
    turnstile_token: str = ""
    so_token: str = ""
...
EDITABLE_FILE_PPT_PROMPT = """I need you to build an editable PPT based on the user's requirements. You may use an Agent to do this. Do not keep asking the user questions; you may fill in and directly execute the content style, layout, color scheme, content structure and page information yourself. The overall flow is:
1. Using image generation, create a polished product-introduction PPT with 5-6 pages.
2. Split all of the images and shape assets above into individual PNGs, one image per asset, with none missing, so that I can reassemble the assets directly in the PPT; no text.
3. Using all of the images and shape assets above, recreate the presentation PPT you first generated. I need it in an editable PPT format; the main parts must be recreated and inserted separately, and the text must be editable.
In the end, just give me a single PPT file plus a zip archive of all the assets produced along the way."""
EDITABLE_FILE_PSD_PROMPT = "Generate this image for me and split this poster into several images, including the background, without moving any element, so I can use them directly without dragging. The base color is white; do not use a fake transparent background. Then merge the split images above into a single PSD file, remove the white background, do not change the position of each layer, keep each element on its corresponding layer at its original position, keep a separate layer for each element, and in the end just output the PSD file plus a zip of each layer."
...
# Real image file ID format: file_00000000 + 24 hex chars (32 chars total)
...
# Content-policy violation keywords (various ways the upstream refuses to generate images)
...
def _is_content_policy_error(error_msg: str) -> bool:
    """Check whether the error message is a content-policy violation."""
    if not error_msg:
        return False
...
class OpenAIBackendAPI:
    ...
    Notes:
    - When `access_token` is provided, chat and model listing use the authenticated path
      e.g. `/backend-api/sentinel/chat-requirements`, `/backend-api/conversation`
    - When `access_token` is not provided, the anonymous path is used
    ...
    - Protocol-compatibility conversions live in `services.protocol`
    """
    ...
    def __init__(self, access_token: str = "") -> None:
        ...
        Args:
        - `access_token`: optional. When provided, use the authenticated path; otherwise use the anonymous path.
        """
    ...
    def _headers(self, path: str, extra: Optional[Dict[str, str]] = None) -> Dict[str, str]:
        """Build the request headers, adding the target path/route required by the web frontend."""
    ...
    def get_user_info(self) -> Dict[str, Any]:
        """Get account info for the current token."""
        ...
        result = {
            ...
            "status": "active" if image_quota_unknown and plan_type.lower() != "free" else ("rate_limited" if quota == 0 else "active"),
        }
        logger.debug({
    ...
    def _bootstrap_headers(self) -> Dict[str, str]:
        """Build the headers for the homepage warm-up request."""
        return {
            "User-Agent": self.user_agent,
    ...
    def _build_requirements(self, data: Dict[str, Any], source_p: str = "") -> ChatRequirements:
        """Normalize the sentinel response into the token set needed for later conversation."""
    ...
    def _conversation_headers(self, path: str, requirements: ChatRequirements) -> Dict[str, str]:
        """Build the conversation SSE headers based on the current requirements."""
        headers = {
    ...
    def _api_messages_to_conversation_messages(self, messages: list[Dict[str, Any]]) -> list[Dict[str, Any]]:
        """Convert standard chat messages into the messages needed for a web conversation."""
    ...
    def _image_headers(self, path: str, requirements: ChatRequirements, conduit_token: str = "", accept: str = "*/*") -> \
            Dict[str, str]:
        """Build the headers for the image path."""
        headers = {
            "Content-Type": "application/json",
            "Accept": accept,
            "OpenAI-Sentinel-Chat-Requirements-Token": requirements.token,
    ...
    def _prepare_image_conversation(self, prompt: str, requirements: ChatRequirements, model: str) -> str:
        """Prepare the conduit token for image generation."""
    ...
    def _list_recent_conversations(self, limit: int = 5, timeout_secs: float = 10.0) -> list[Dict[str, Any]]:
        """List recent conversations, sorted by update time descending.

        When the SSE stream is too short and the conversation_id is lost, this method
        can find the most recently created conversation to recover the conversation_id.
        """
    ...
    def find_conversation_by_prompt(self, prompt: str, started_at: float, timeout_secs: float = 10.0) -> str:
        """Find a matching conversation_id from the recent conversation list using the prompt and start time.

        Use this method to recover when the SSE stream is too short and the conversation_id is lost.
        It matches the most likely conversation by comparing prompt keywords and timestamps.

        Args:
            prompt: the user-entered prompt text
            started_at: the request start timestamp (epoch seconds)
            timeout_secs: request timeout in seconds

        Returns:
            the matching conversation_id, or an empty string if none is found
        """
        items = self._list_recent_conversations(limit=10, timeout_secs=timeout_secs)
        if not items:
            return ""
        # Filter conversations created before or near started_at (up to 5 minutes earlier)
        # ChatGPT's updated_at is usually later than the actual request time
        prompt_lower = str(prompt or "").lower().strip()
        best_match = ""
        best_score = 0.0
        for item in items:
            # item may be a full conversation object or a summary
            conv_id = str(item.get("id") or item.get("conversation_id") or "")
            if not conv_id:
                continue
            # Check the time range: the conversation's updated_at should be at or after the request start time
        ...
        if best_match and best_score > 0.1:
            ...
            return best_match
        # If no title matches, return the latest (most recent) conversation
        for item in items:
            conv_id = str(item.get("id") or item.get("conversation_id") or "")
            updated_at = float(item.get("update_time") or item.get("updated_at") or 0)
    ...
    def _editable_prompt(fixed_prompt: str, user_prompt_text: str) -> str:
        ...
        return fixed_prompt if not extra else fixed_prompt + "\n\nAdditional user requirements below; incorporate and execute them directly:\n" + extra
    ...
    def _extract_image_reference_ids(cls, payload: Any) -> tuple[list[str], list[str]]:
        ...
        def walk(value: Any) -> None:
            if isinstance(value, str):
                # Only extract real image file IDs (file_00000000... format) and file-service:// URIs
    ...
    def _find_content_policy_error_in_conversation(data: Dict[str, Any]) -> str:
        """Find a content-policy violation error message in the conversation document.

        When the upstream refuses to generate an image, the error message appears in the assistant message text.
        This method iterates over all assistant/tool messages, checking for content-policy violation keywords,
        and returns that message text (truncated to 500 chars) if matched, otherwise an empty string.
        """
        mapping = data.get("mapping") or {}
    ...
    def _poll_image_results(
        ...
        while _remaining() > 0:
            attempt += 1
            # On each poll, check /backend-api/tasks/ for errors (log only, do not interrupt)
            ...
            try:
                ...
                # A failed tasks query does not affect the normal polling flow
                logger.debug({
            ...
            # Check whether the conversation text contains a content-policy violation error
            # When the upstream refuses to generate an image, the error appears in the conversation document's assistant message,
            # rather than in the task error structure of /backend-api/tasks/.
            # If a content-policy violation is detected while no image file ID is found, interrupt polling immediately.
            ...
            if file_ids or sediment_ids:
                if not config.image_check_before_hit_enabled:
                    # Check-before-hit disabled: return the first-found file_ids directly
                ...
                if not config.image_settle_enabled:
                    # Double-confirm disabled: return the first-found file_ids directly
        ...
        exc = ImagePollTimeoutError(
            f"ChatGPT image generation timeout (waited {timeout_secs} seconds). "
            f"You can raise the image_poll_timeout_secs threshold in config.json; "
            f"this may also be caused by the account being rate-limited or the image queue being congested."
        )
        if last_task_error:
    ...
    def _get_attachment_download_url(self, conversation_id: str, attachment_id: str) -> str:
        """Get the download URL via the conversation attachment API."""
        path = f"/backend-api/conversation/{conversation_id}/attachment/{attachment_id}/download"
        response = self.session.get(self.base_url + path, headers=self._headers(path, {"Accept": "application/json"}),
                                    timeout=60)
        ensure_ok(response, path)
    ...
    def _query_backend_tasks(
        ...
        """Query the /backend-api/tasks/ API for async task status and error info.
        ...
        Returns:
        - A list of tasks, each containing fields such as image_gen_message.
        """
    ...
    def check_task_error(self, task: Dict[str, Any]) -> tuple[bool, str, Dict[str, Any]]:
        """Check whether a single task contains a structured error.

        Decided via the following fields (without relying on text matching):
        - image_gen_message.metadata.is_error == True
        - image_gen_message.author.role == "assistant" (rather than "tool")
        - image_gen_message.content.content_type == "text" (rather than "multimodal_text")
    ...
    def _resolve_image_urls(self, conversation_id: str, file_ids: list[str], sediment_ids: list[str]) -> list[str]:
        """Resolve an image result id into a downloadable URL."""
        urls = []
    ...
    def resolve_conversation_image_urls(
        ...
        # When both check-before-hit and settle are disabled and SSE already provided file_ids,
        # skip polling and resolve the URL directly, saving initial_wait + polling time.
        ...
        if poll and conversation_id:
            ...
            try:
                ...
                # If polling times out and there is a task error (e.g. moderation block), raise ImageContentPolicyError
                # instead of ImagePollTimeoutError, so callers can distinguish a real timeout from an upstream refusal
    ...
    def _bootstrap(self) -> None:
        """Warm up the homepage and extract the PoW-related script references."""
        response = self.session.get(
            self.base_url + "/",
    ...
    def _get_chat_requirements(self) -> ChatRequirements:
        """Get the sentinel token needed for conversations in the current mode."""
    ...
    def list_models(self) -> Dict[str, Any]:
        """Return the models available in the current mode, formatted to match OpenAI `/v1/models`."""
        self._bootstrap()

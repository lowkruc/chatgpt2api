...
class CloudflareTempMailProvider(BaseMailProvider):
    ...
    def get_existing_mailbox(self, email: str) -> dict[str, Any]:
        """Use the admin password to obtain the JWT for an existing mailbox, used to query mail."""
        data = self._request("POST", "/admin/get_address", headers={"x-admin-auth": self.admin_password}, payload={"address": email})
        address = str(data.get("address") or "").strip()
        token = str(data.get("jwt") or "").strip()
        if not address or not token:
            raise RuntimeError(f"CloudflareTempMail could not obtain the JWT for existing mailbox {email}")
        return {"provider": self.name, "provider_ref": self.provider_ref, "address": address, "token": token}
...
class DDGMailProvider(BaseMailProvider):
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        ...
        if not ddg_address_part:
            raise RuntimeError("DDG API returned no address field")
        ddg_address = f"{ddg_address_part}@duck.com"

        if _is_ddg_alias_duplicate(ddg_address):
            raise RuntimeError(f"[{self.label}] DDG daily limit reached; alias {ddg_address} already exists, switching mail provider automatically")

        _record_ddg_alias(ddg_address)

        if not self.cf_inbox_jwt:
            raise RuntimeError("DDGMail requires cf_inbox_jwt (the fixed inbox JWT for the DDG forwarding target); please set CF Inbox JWT in the mail config")
...
class CloudMailGenProvider(BaseMailProvider):
    ...
    def _request(
        ...
        if resp.status_code not in expected:
            raise RuntimeError(f"CloudMailGen request failed: {method} {path}, HTTP {resp.status_code}, body={resp.text[:300]}")
        return {} if resp.status_code == 204 else resp.json()
    ...
    def _get_token(self) -> str:
        ...
        if not token:
            raise RuntimeError(f"CloudMailGen genToken returned an unexpected response: {data}")
        with cloudmail_token_lock:
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        if not self.domain:
            raise RuntimeError("CloudMailGen needs at least one domain configured")
...
class TempMailLolProvider(BaseMailProvider):
    ...
    def _request(self, method: str, path: str, params: dict | None = None, payload: dict | None = None, expected: tuple[int, ...] = (200,)):
        ...
        if not isinstance(data, dict):
            raise RuntimeError(f"TempMail.lol {method} {path} returned a non-object structure")
        return data
...
class MoEmailProvider(BaseMailProvider):
    ...
    def _request(self, method: str, path: str, params: dict | None = None, payload: dict | None = None, expected: tuple[int, ...] = (200,)):
        ...
        data = resp.json()
        if not isinstance(data, dict):
            raise RuntimeError(f"MoEmail {method} {path} returned a non-object structure")
        return data
...
class InbucketMailProvider(BaseMailProvider):
    ...
    def _resolve_domain(self) -> str:
        ...
        raise RuntimeError("Inbucket needs at least one domain configured")
...
def create_mailbox(mail_config: dict, username: str | None = None) -> dict:
    ...
    for _ in range(len(enabled)):
        ...
        try:
            ...
            if "DDG daily limit reached" not in last_error:
                raise
        finally:
            provider.close()
    raise RuntimeError(last_error or "None of the enabled mail providers could create a mailbox")
...
def get_existing_mailbox(mail_config: dict, email: str) -> dict:
    ...
    for _ in range(len(enabled)):
        ...
        try:
            ...
            provider.close()
    raise RuntimeError(last_error or "None of the enabled mail providers could query the existing mailbox")

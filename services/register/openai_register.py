...
def build_sentinel_token(session: requests.Session, device_id: str, flow: str) -> str:
    """Request a sentinel token and return the sentinel header string (kept for backward compatibility)."""
...
class PlatformRegistrar:
    ...
    def _register_user(self, email: str, password: str, index: int) -> None:
        ...
        if resp is None or resp.status_code != 200:
            ...
            if data.get("message") == "Failed to create account. Please try again.":
                step(index, "Registration failed: the email domain is likely banned for abuse; please use a different email domain", "yellow")
    ...
    def _validate_otp(self, code: str, index: int) -> None:
        step(index, f"Verifying verification code {code}")
        ...
        step(index, "Verification code verified")
    ...
    def _create_account(self, name: str, birthdate: str, index: int) -> None:
        ...
        if resp is None or resp.status_code not in (200, 302):
            data = _response_json(resp) if resp is not None else {}
            if data.get("message") == "Failed to create account. Please try again.":
                step(index, "Account creation failed: the email domain is likely banned for abuse; please use a different email domain", "yellow")
            detail = f", detail={json.dumps(data, ensure_ascii=False)}" if data else ""
    ...
    def register(self, index: int) -> dict:
        ...
        if not email:
            raise RuntimeError("The mail service did not return an address")
        label = str(mailbox.get("label") or "")
        step(index, f"Mailbox created [{label}]: {email}")
        password = _random_password()
        first_name, last_name = _random_name()
        self._platform_authorize(email, index)
        self._register_user(email, password, index)
        self._send_otp(index)
        step(index, "Waiting for the registration verification code")
        code = wait_for_code(mailbox)
        if not code:
            raise RuntimeError("Timed out waiting for the registration verification code")
...
def worker(index: int) -> dict:
    ...
    try:
        ...
        if refresh_result.get("errors"):
            step(index, f"Account saved, but refreshing status did not succeed yet; you can retry later: {refresh_result['errors']}", "yellow")
        with stats_lock:
            stats["done"] += 1
            stats["success"] += 1
            avg = (time.time() - stats["start_time"]) / stats["success"]
        log(f'{result["email"]} registered successfully in {cost:.1f}s; global average per account is {avg:.1f}s', "green")
        ...
        with stats_lock:
            stats["done"] += 1
            stats["fail"] += 1
        log(f"Task {index} registration failed in {cost:.1f}s; reason: {e}", "red")

...
def _public_task(task: dict[str, Any]) -> dict[str, Any]:
    ...
    if task.get("status") in (TASK_STATUS_RUNNING, TASK_STATUS_QUEUED):
        if task.get("status") == TASK_STATUS_RUNNING:
            # RUNNING state only starts timing after started_ts is set (image_stream_resolve_start)
            base_ts = task.get("started_ts")
        else:
            # QUEUED state starts timing from created_ts (waiting in queue)
...
class ImageTaskService:
    ...
    def _run_task(
        ...
        # Create a progress callback that updates the task status after each step completes
        def progress_callback(step: str) -> None:
            if step == "image_stream_resolve_start":
                self._update_task(key, started_ts=time.time())
            self._update_task(key, progress=step)
        # Add the progress callback to the payload (the handler extracts and passes it to ConversationRequest)
        ...
        try:
            ...
            if not isinstance(data, list) or not data:
                ...
                if upstream:
                    ...
                    message = "No available accounts in the pool, or all accounts are rate-limited; please check the pool status (account quota, whether banned, whether the image generation limit is reached)"
    ...
    def _recover_unfinished_locked(self) -> bool:
        ...
        for task in self._tasks.values():
            if task.get("status") in UNFINISHED_STATUSES:
                ...
                task["error"] = "The service has restarted; unfinished image tasks were interrupted"
                task["updated_at"] = _now_iso()
                changed = True
        return changed
    ...
    def resume_poll(
        ...
        """Resume polling for a timed-out task, waiting an extra extra_timeout_secs seconds."""
        owner = _owner_id(identity)
        ...
        with self._lock:
            ...
            # Reset the task status to running
            self._update_task(key, status=TASK_STATUS_RUNNING, error="")

        # Start a new thread to keep polling
        thread = threading.Thread(
    ...
    def _run_resume_poll(
        ...
        """Background thread: keep polling for image results of an existing conversation_id."""
        started = time.time()
        try:
            ...
            if not file_ids and not sediment_ids:
                raise RuntimeError(
                    f"Still no image result after waiting an extra {extra_timeout_secs} seconds."
                )
            ...
            if not image_urls:
                raise RuntimeError("Failed to resolve the image URL")

            image_items = [
                {"b64_json": __import__("base64").b64encode(image_data).decode("ascii")}
                for image_data in backend.download_image_bytes(image_urls)
            ]
            # Get the task's original prompt (inferred from _public_task's mode)
            with self._lock:
                task = self._tasks.get(key)
            ...
            data = format_image_result(
                image_items,
                "",  # prompt no longer matters; the result is already obtained
                "b64_json",
                "",
                int(time.time()),
            )["data"]

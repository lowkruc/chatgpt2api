...
def _load_settings() -> LoadedSettings:
    ...
    if _is_invalid_auth_key(auth_key):
        raise ValueError(
            "❌ auth-key is not set!\n"
            "Set it via the CHATGPT2API_AUTH_KEY environment variable, or fill in auth-key in config.json."
        )
...
class ConfigStore:
    def __init__(self, path: Path):
        ...
        if _is_invalid_auth_key(self.auth_key):
            raise ValueError(
                "❌ auth-key is not set!\n"
                "Fix it in any of the following ways:\n"
                "1. Add it to Render's Environment variables:\n"
    ...
    def image_settle_enabled(self) -> bool:
        """Image double-confirm: after finding file_ids, wait a while and confirm again."""
    ...
    def image_check_before_hit_enabled(self) -> bool:
        """Check before hit: confirm file_ids exist via polling before returning, instead of relying only on SSE events."""
    ...
    def image_settle_secs(self) -> float:
        """Double-confirm wait time (seconds)."""
        try:

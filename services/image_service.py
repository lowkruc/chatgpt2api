...
def compress_images(quality: int = 60) -> dict:
    """Recompress all images and return the amount of space saved."""
    saved = 0
    count = 0
...
def delete_to_target(target_free_mb: int, dry_run: bool = False) -> dict:
    """Delete the oldest images until free space reaches target_free_mb."""
    import shutil
    usage = shutil.disk_usage(config.images_dir)
...
def _auto_cleanup_worker(stop_event: threading.Event) -> None:
    """Background thread: check storage every 30 minutes and auto-clean the oldest images when below the threshold."""
    import shutil

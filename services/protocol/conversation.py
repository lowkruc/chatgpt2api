...
def is_tls_connection_error(message: str) -> bool:
    """Detect TLS/SSL connection errors, which can usually be resolved by retrying."""
    text = str(message or "").lower()
    return (
        "curl: (35)" in text
...
def is_connection_timeout_error(message: str) -> bool:
    """Detect connection-timeout errors (e.g. curl 28), which can be resolved by a short same-account retry."""
    text = str(message or "").lower()
    return (
        "curl: (28)" in text
...
# Detect partial tool-call JSON returned by the model (e.g. {"size":"1920x1088","n":1})
# This JSON contains the image-generation tool parameters but no actual generated image
...
def is_model_text_reply_instead_of_image(message: str) -> bool:
    """Detect whether the model returned a text reply (containing tool-call JSON) instead of an actual generated image.

    When the upstream ChatGPT fails to trigger the image-generation tool, it returns descriptive text
    that may include JSON parameters (such as prompt, referenced_image_ids, size/n, etc.).
    This case should be treated as "upstream did not generate an image" rather than a "content-policy violation".

    Detects two patterns:
    1. Full tool-call JSON (with referenced_image_ids)
    ...
    if REFERENCED_IMAGE_IDS_RE.search(message):
        return True
    # Detect partial tool-parameter JSON (the model returned tool parameters but did not trigger the tool)
...
# Used to filter out non-image file IDs (e.g. file_upload_business_upsell)
...
def extract_conversation_ids(payload: str) -> tuple[str, list[str], list[str]]:
    ...
    # Only extract real image file IDs (file_00000000... format), filtering out non-image file IDs (e.g. file_upload_business_upsell)
...
def _get_detailed_error_from_tasks(
    ...
    """Fetch structured error info from the /backend-api/tasks/ API.

    When the SSE stream detects a moderation block, poll the tasks API for detailed error text.
    It decides using structured fields (metadata.is_error, author.role, content.content_type),
    rather than relying on fragile text matching.

    Args:
    - `backend`: an OpenAIBackendAPI instance.
    - `conversation_id`: the conversation ID.
    - `timeout_secs`: request timeout in seconds.
    - `wait_secs`: seconds to wait for the task to be created. Set to 0 to skip waiting.

    Returns:
    - The detailed error text, or an empty string if none is found.
    """
...
def stream_image_outputs(
    ...
    if message and not file_ids and not sediment_ids and last.get("blocked"):
        # Try to get detailed error info from /backend-api/tasks/
    ...
    # Detect whether the model returned a text description (with referenced_image_ids) instead of an actual image
    # This means the model triggered the image-generation tool call but the SSE dropped before the tool finished,
    # so the image may still be generating asynchronously. A more aggressive polling strategy is needed to get the result.
    ...
    # When a text reply is detected but the conversation_id is lost, try to recover it from the recent conversation list
    # When the SSE stream is too short (the model returns text instead of triggering the image tool), the conversation_id may not be captured,
    # but the image is already generating asynchronously upstream. Recover the conversation_id by listing recent conversations.
    ...
    # Before polling for the image, first check /backend-api/tasks/ for a moderation block
    # This avoids an unnecessary long polling timeout
    # Note: when should_poll_for_image is True or a text reply is detected,
    # even if tasks reports an "error" we must not return directly — because the upstream may mark the tool-call JSON parameters
    # (e.g. {"size":"1792x1024","n":1}) as is_error while the image is actually still generating asynchronously.
    # In this case, keep polling for the image.
    detailed_error = ""
    ...
    # When a text reply (with referenced_image_ids) is detected, use a longer timeout to poll for the image result.
    # Because the upstream may run image generation as an async task, and the SSE stream drops before the tool finishes,
    # so the conversation document does not yet contain the image tool's response record.
    poll_timeout = config.image_poll_timeout_secs
    if is_text_reply and conversation_id:
        # In the text-reply case the image may still be generating asynchronously, so use a longer timeout (default 120s -> extra 180s = 300s)
    ...
    try:
        ...
        # When a text reply is detected, a task error should not be treated directly as a content-policy violation,
        # because the image may still be generating asynchronously in the background
        ...
        # When a text reply is detected, a transient network error on the first poll should not interrupt directly,
        # because the image may still be generating asynchronously in the background, and later retry polls will keep trying.
        if is_text_reply and conversation_id:
            logger.warning({
    ...
    if message:
        # Detect whether the model returned a text description (with referenced_image_ids) instead of an actual image
        # This means the model triggered the image-generation tool call but the SSE dropped before the tool finished.
        # In this case, try polling for the image result again instead of treating the text as the final output.
        # When is_text_reply but the conversation_id is lost, try to recover it from the recent conversation list
        if is_text_reply and not conversation_id:
            try:
                import time as _time
                recovered_id = backend.find_conversation_by_prompt(
        ...
        if is_text_reply and conversation_id:
            ...
            # In the text-reply case, the image may take 4-5 minutes to finish generating asynchronously.
            # Use a 300s timeout and allow multiple retries to avoid exiting early due to transient network issues.
            ...
            for poll_attempt in range(1, MAX_POLL_RETRIES + 1):
                try:
                    ...
                    break  # Polling succeeded; exit the retry loop
                except Exception as exc:
                    error_str = str(exc)
                    is_transient = (
                    ...
                    # If retries remain and it is not a timeout/content-violation error, keep retrying
    ...
    # Fallback: when the message is empty and image URL resolution fails, first try one short-delay retry poll
    # then raise an explicit error instead of giving the caller a vague "upstream completed without generating images"
    logger.warning({
    ...
    # When should_poll_for_image is True but the conversation_id is lost, try to recover it
    ...
    if should_poll_for_image and conversation_id:
        # The image may still be processing asynchronously (the upstream SSE stream ended before generation finished).
        # Use a 300s timeout and allow multiple retries to avoid exiting early due to transient network issues or the image not yet being submitted.
        retry_poll_timeout = max(config.image_poll_timeout_secs, 300)
        ...
        # Still failing after retries; yield the error message
        ...
        # When the conversation_id is also empty (extremely short SSE stream, no conversation ID captured),
...
def _generate_single_image(
    ...
    """Run the generation logic for a single image (with retries) and return the result list.

    This function runs in a separate thread, each thread using a different account,
    enabling parallel image generation and avoiding serial timeout blocking.
    """
    # Max retries when the model returns text instead of an image
    MAX_TEXT_REPLY_RETRIES = 3
    # Max retries for TLS connection errors
    MAX_TLS_RETRIES = 3
    # Max retries for connection-timeout errors (short same-account retry)
    MAX_CONN_TIMEOUT_RETRIES = 3
    # Max retries for polling-timeout errors (retry with a different account)
    ...
    while True:
        ...
        try:
            ...
            # Polling timeout: retry with a different account
            ...
            error_text = str(exc)
            # If the model returned text instead of an image, try retrying with a different account
            ...
            if not emitted_for_token and is_token_invalid_error(last_error):
                ...
                continue
            # TLS/SSL connection error: retry automatically
            if not emitted_for_token and is_tls_connection_error(last_error):
                ...
                if tls_retry_count <= MAX_TLS_RETRIES:
                    ...
                    continue
            # Connection-timeout error (curl 28): short same-account retry without switching accounts
...
def stream_image_outputs_with_pool(request: ConversationRequest) -> Iterator[ImageOutput]:
    """Generate multiple images in parallel, each using its own thread and account, without blocking each other."""
    ...
    if request.n <= 1:
        # Single image: run directly (no thread-pool overhead needed)
        outputs = _generate_single_image(request, 1, 1)
        for output in outputs:
            yield output
        return

    # Multiple images: run in parallel or serially depending on the config
    ...
    # Yield results: drop the index-order constraint so a low-index failure no longer blocks high-index successes
    emitted = False
    last_error = ""

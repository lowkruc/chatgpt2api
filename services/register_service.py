...
class RegisterService:
    ...
    def stop(self) -> dict:
        with self._lock:
            ...
            self._save()
            self._append_log("Stop requested; waiting for the currently running task to finish", "yellow")
            return self.get()
    ...
    def _target_reached(self, cfg: dict, submitted: int) -> bool:
        ...
        if mode == "quota":
            ...
            self._append_log(f"Checking pool: current active accounts={metrics['current_available']}, current remaining quota={metrics['current_quota']}, target quota={cfg.get('target_quota')}, {'skipping registration' if reached else 'continuing registration'}", "yellow")
            return reached
        if mode == "available":
            reached = metrics["current_available"] >= int(cfg.get("target_available") or 1)
            self._append_log(f"Checking pool: current active accounts={metrics['current_available']}, target accounts={cfg.get('target_available')}, current remaining quota={metrics['current_quota']}, {'skipping registration' if reached else 'continuing registration'}", "yellow")
            return reached

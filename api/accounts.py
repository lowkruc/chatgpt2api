...
class OAuthLoginStartRequest(BaseModel):
    """Start the OAuth bridge. email_hint is optional and only prefills the email on the OpenAI login page."""
    email_hint: str = ""


class OAuthLoginFinishRequest(BaseModel):
    """Submit the callback. callback can be either a full URL or just the code."""
    session_id: str = ""
    callback: str = ""
...
def create_router() -> APIRouter:
    ...
    async def update_user_key(
        ...
        if not updates:
            raise HTTPException(status_code=400, detail={"error": "No changes detected yet; please edit before saving"})
        try:
    ...
    async def re_login_accounts(body: AccountRefreshRequest, authorization: str | None = Header(default=None)):
        """Run the password re-login flow for the selected accounts (password login -> code login -> refresh token)."""
        require_admin(authorization)
        access_tokens = [str(token or "").strip() for token in body.access_tokens if str(token or "").strip()]
        if not access_tokens:
            raise HTTPException(status_code=400, detail={"error": "access_tokens is required"})
    ...
    async def export_accounts(body: AccountExportRequest, authorization: str | None = Header(default=None)):
        ...
        if not items:
            raise HTTPException(
                status_code=400,
                detail={"error": "No complete account to export; access_token, refresh_token and id_token are all required"},
            )
    ...
    async def start_oauth_login(
        ...
        """Register a PKCE session and return an authorize URL for the user's browser to open."""
        require_admin(authorization)
        try:
    ...
    async def finish_oauth_login(
        ...
        """Take the callback URL / code captured from the user's browser, exchange the token triplet and persist it."""
        require_admin(authorization)
        # Input logging: truncate sensitive fields, keep only the first few chars for debugging without leaking secrets

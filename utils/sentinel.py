"""Utilities to generate and request OpenAI Sentinel Tokens (PoW).

Used by flows that require a sentinel token, such as password login and registration.
"""
...
class SentinelTokenGenerator:
    """Sentinel Token generator (PoW - Proof of Work)."""
...
# ── Default User-Agent and sec-ch-ua ──────────────────────────────
...
def build_sentinel_token(
    ...
    *,
    ...
    """Request a sentinel token and return (sentinel_header_value, oai_sc_cookie_value).

    Args:
        session: curl_cffi Session instance
        device_id: device ID
        flow: flow identifier (e.g. "password_verify", "username_password_create", etc.)
    ...
    Raises:
        RuntimeError: the sentinel request failed
    """

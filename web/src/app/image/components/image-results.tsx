...
// Blob URL cache: avoid huge base64 strings in the DOM by using short blob: URLs instead
...
async function downloadStoredImage(image: StoredImage, index: number) {
  ...
  try {
    ...
    // If fetch fails, try opening directly in a new window
    if (image.url) {
...
export function ImageResults({
  ...
  // Only start the timer when there are loading images, to avoid needless re-renders when idle
  ...
  const updateImageDimensions = (id: string, width: number, height: number) => {
    ...
    // Store in a ref to avoid React re-renders and eliminate cascading re-renders
    ...
    return (
      <div className="flex h-full min-h-[260px] items-center justify-center text-center sm:min-h-[420px]">
        <div className="w-full max-w-4xl">
          ...
          >
            Keep local history and task status in the same window, and start new stateless edits from existing result images.
          </p>
        </div>
      </div>
    );
              ...
              <div className="flex justify-end">
                <div className="max-w-[90%] px-1 py-1 text-[14px] leading-6 text-stone-900 sm:max-w-[82%] sm:text-[15px] sm:leading-7">
                  <div className="mb-1.5 flex flex-wrap justify-end gap-2 text-[11px] text-stone-400 sm:mb-2">
                    ...
                    <span>
                      {turn.mode === "edit" ? "Edit" : "Text-to-image"}
                    </span>
                ...
                <div className="w-full p-1">
                  {turn.referenceImages.length > 0 ? (
                    <div className="mb-4 flex flex-col items-end">
                      <div className="mb-3 text-xs font-medium text-stone-500">Reference images this round</div>
                  ...
                  <div className="mb-3 flex flex-wrap items-center gap-1.5 text-[11px] text-stone-500 sm:mb-4 sm:gap-2 sm:text-xs">
                    ...
                    {turn.status === "queued" ? (
                      <span className="rounded-full bg-amber-50 px-3 py-1 text-amber-700">Waiting for earlier tasks in this conversation to finish</span>
                    ) : null}
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:block sm:columns-2 sm:gap-4 sm:space-y-4 xl:columns-3">
                    {turn.images.map((image, index) => {
                      ...
                      if (image.status === "error") {
                        const isTimeoutError = image.error?.toLowerCase().includes("timeout") && image.taskId;
                        return (
...
const PROGRESS_LABELS: Record<string, string> = {
  getting_account: "Checking available account",
  uploading: "Uploading image",
  bootstrapping: "Warming up homepage",

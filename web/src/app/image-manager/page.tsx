...
function ImageManagerContent() {
  ...
  const handleDeleteTag = async (tag: string) => {
    try {
      ...
      toast.success(`Tag "${tag}" deleted, affecting ${result.removed_from} images`);
    } catch (error) {
    ...
    <section className="space-y-5">
        ...
        <DialogContent className="sm:max-w-md rounded-2xl">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-stone-400">images before this</span>
            </div>
            <p className="text-xs text-stone-500">This action cannot be undone; it permanently deletes all matching-date images and their thumbnails.</p>
          </div>
          <DialogFooter>
      ...
      <Dialog open={deleteMode === "selected" || deleteMode === "filtered"} onOpenChange={(open) => (!open ? setDeleteMode(null) : null)}>
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            <DialogTitle>{deleteMode === "filtered" ? "Delete matching-date images" : "Delete selected images"}</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-stone-600">
      ...
      <Dialog open={Boolean(tagDeleteTarget)} onOpenChange={(open) => { if (!open) setTagDeleteTarget(null); }}>
        <DialogContent className="max-w-sm rounded-2xl">
          ...
          <p className="text-sm text-stone-600">
            Delete the tag <span className="font-semibold">"{tagDeleteTarget}"</span>? It will be removed from all images.
          </p>
          <DialogFooter>

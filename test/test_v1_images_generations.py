...
class ImageGenerationsTests(unittest.TestCase):
    def test_image_generation_http(self):
        """Test the non-streaming HTTP call for image generation."""
        response = requests.post(
            f"{BASE_URL}/v1/images/generations",
            headers={"Authorization": f"Bearer {AUTH_KEY}"},
            json={
                "model": "gpt-image-2",
                "prompt": "I want to create a Nanjing city promotional poster.",
                "n": 1,
    ...
    def test_image_generation_stream_http(self):
        """Test the streaming HTTP call for image generation."""
        response = requests.post(
            f"{BASE_URL}/v1/images/generations",
            headers={"Authorization": f"Bearer {AUTH_KEY}"},
            json={

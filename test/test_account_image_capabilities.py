...
class AccountCapabilityTests(unittest.TestCase):
    ...
    def test_get_available_access_token_filters_by_plan_type(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            service.add_account_items(
                [
                    {"access_token": "token-plus", "type": "Plus", "status": "active", "quota": 3},
                    {"access_token": "token-pro", "type": "Pro", "status": "active", "quota": 3},
                ]
            )
    ...
    def test_refresh_accounts_can_remove_invalid_token_without_confirmation_delay(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_dir:
                service = AccountService(JSONStorageBackend(Path(tmp_dir) / "accounts.json"))
                service.add_account_items([{"access_token": "invalid-token", "status": "active"}])

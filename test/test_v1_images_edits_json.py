...
class ImageEditsJsonApiTests(unittest.TestCase):
    ...
    def test_json_model_omitted_uses_existing_default_logic(self):
        response = self.client.post("/v1/images/edits", headers=AUTH_HEADERS, json={"prompt": "no model passed", "image": PNG_DATA_URL})
    ...
    def test_image_edit_accepts_json_image_url(self):
        response = self.client.post(
            ...
            json={
                "model": "gpt-image-2",
                "prompt": "Change the image to a night-scene style",
                "n": 1,
                "size": "1024x1536",
    ...
    def test_image_edit_accepts_json_multiple_images_and_b64_json(self):
        response = self.client.post(
            ...
            headers=AUTH_HEADERS,
            json={
                "prompt": "Combine the two images into a poster",
                "images": [
                    PNG_DATA_URL,
    ...
    def test_image_edit_keeps_original_multipart_multiple_image_logic(self):
        response = self.client.post(
            "/v1/images/edits",
            headers=AUTH_HEADERS,
            data={"prompt": "multipart multi-image still works", "model": "gpt-image-2", "n": "1"},
            files=[
    ...
    def test_image_edit_rejects_remote_json_url(self):
        response = self.client.post(
            "/v1/images/edits",
            headers=AUTH_HEADERS,
            json={"prompt": "remote image fetch not allowed", "images": [{"image_url": "https://example.com/a.png"}]},
        )
        self.assertEqual(response.status_code, 400, response.text)
        self.assertIn("remote image URLs are not supported", response.text)

    def test_image_edit_rejects_json_n_out_of_range(self):
        response = self.client.post("/v1/images/edits", headers=AUTH_HEADERS, json={"prompt": "n out of range", "n": 5, "image": PNG_DATA_URL})
        self.assertEqual(response.status_code, 400, response.text)
